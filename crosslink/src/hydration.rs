...
pub fn hydrate_to_sqlite(cache_dir: &Path, db: &Database) -> Result<HydrationStats> {
    ...
    struct SavedChildren {
        ...
        comments: Vec<(i64, i64, Option<String>, Option<String>, String, String, String, Option<String>, Option<String>, Option<String>)>,
        deps: Vec<(i64, i64)>,
        relations: Vec<(i64, i64)>,
        time_entries: Vec<(i64, i64, String, Option<String>, Option<i64>)>,
    ...
    let saved_children = if preserved_ids.is_empty() {
        ...
        let id_placeholders: String = preserved_ids.iter().map(|id| id.to_string()).collect::<Vec<_>>().join(",");

        let labels = db.conn
            .prepare(&format!("SELECT issue_id, label FROM labels WHERE issue_id IN ({})", id_placeholders))?
        ...
        SavedChildren { labels, comments, deps, relations, time_entries, milestone_issues }
    };
    ...
    let result = db.transaction(|| {
        ...
        for (id, issue_id, uuid, author, content, created_at, kind, trigger_type, intervention_context, driver_key_fingerprint) in &saved_children.comments {
            db.insert_hydrated_comment(
                *id, *issue_id, uuid.as_deref(), author.as_deref(), content,
                created_at, kind, trigger_type.as_deref(),
                intervention_context.as_deref(), driver_key_fingerprint.as_deref(),
            )?;
            stats.comments += 1;
        }
        ...
        for (id, issue_id, started_at, ended_at, duration_seconds) in &saved_children.time_entries {
            db.insert_hydrated_time_entry(*id, *issue_id, started_at, ended_at.as_deref(), *duration_seconds)?;
        }
