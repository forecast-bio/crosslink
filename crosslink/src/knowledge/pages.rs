...
impl KnowledgeManager {
    ...
    pub(crate) fn safe_page_path(&self, slug: &str) -> Result<PathBuf> {
        ...
        match (
            ...
            (Ok(canonical_cache), Some(canonical_parent)) => {
            ...
            // Cache dir itself cannot be canonicalized (not yet created) —
            // the slug already passed syntactic validation above, so this
            // is safe. The parent-not-canonicalized case (symlink or missing
            // intermediate dir) is also covered by the slug validation.
            _ => {}
        }
        Ok(path)
    }
