...
fn test_claim_and_release_lock() {
    ...
    let claimed = manager.claim_lock(&agent, 1, None, LockMode::Normal).unwrap();
    assert!(claimed);

    // Claiming again for same agent should return false (already held by self)
    let claimed_again = manager.claim_lock(&agent, 1, None, LockMode::Normal).unwrap();
...
fn test_claim_lock_already_locked_by_other_fails() {
    ...
    // Agent 1 claims
    manager.claim_lock(&agent1, 5, None, LockMode::Normal).unwrap();
...
fn test_claim_lock_force_steals() {
    ...
    manager.claim_lock(&agent1, 7, None, LockMode::Normal).unwrap();

    // Agent 2 steals with force=true
    let stolen = manager.claim_lock(&agent2, 7, None, LockMode::Steal).unwrap();
    assert!(stolen);
...
fn test_release_lock_by_different_agent_fails() {
    ...
    manager.claim_lock(&agent1, 3, None, LockMode::Normal).unwrap();
...
fn test_release_lock_by_different_agent_with_force() {
    ...
    manager.claim_lock(&agent1, 4, None, LockMode::Normal).unwrap();
...
fn test_find_stale_locks_v2_uses_helper() {
    ...
    let manager = SyncManager::new(&crosslink_dir).unwrap();
    let stale = manager.find_stale_locks_v2(chrono::Duration::minutes(30)).unwrap();
...
fn test_lock_mode_enum_in_claim_and_release() {
    ...
    assert!(manager
        .release_lock(&agent1, 100, LockMode::Steal)
        .unwrap());
}
