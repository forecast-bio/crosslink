...
use crate::commands::config_registry::{
    ConfigGroup, ConfigType, WalkthroughCore, REGISTRY,
};
    ...
    let result = (|| -> Result<()> {
        loop {
            ...
            if let Event::Key(key) = event::read().context("Failed to read terminal event")? {
                ...
                match key.code {
                    KeyCode::Up | KeyCode::Char('k') if !app.core.is_confirm_screen() => app.move_up(),
                            ...
                            && !app.is_alias_screen() =>
                    {
                        ...
                        if let Some(gi) = app.core.current_group_idx() {
                            if app.core.group_cursor < app.core.group_keys[gi].len() {
                                ...
                                let options = WalkthroughCore::options_for_key(reg_idx);
                                if !options.is_empty() {
                                    let current = app.core.group_selections[gi][app.core.group_cursor];
                                    app.core.group_selections[gi][app.core.group_cursor] = if current == 0 {
                                        options.len() - 1
                                    } else {
                                        current - 1
                                    };
...
pub(super) fn apply_tui_choices(config: &mut serde_json::Value, choices: &TuiChoices) -> Result<()> {
    let obj = config
        .as_object_mut()
