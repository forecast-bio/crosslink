...
// Embed skill files — auto-generated by build.rs from resources/antigravity/skills/
...
impl std::str::FromStr for AgentRuntime {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s {
            ...
            "both" => Ok(Self::Both),
            _ => anyhow::bail!("Unknown runtime '{s}'. Valid: claude, antigravity, both"),
        }
    }
}
...
fn managed_files(python_prefix: &str, runtime: AgentRuntime) -> Vec<(String, String)> {
    ...
    if runtime.wants_antigravity() {
        files.extend([
            (
                ".agents/hooks/prompt-guard.py".into(),
                PROMPT_GUARD_PY.into(),
            ),
            (
                ".agents/hooks/post-edit-check.py".into(),
                POST_EDIT_CHECK_PY.into(),
            ),
            (
                ".agents/hooks/session-start.py".into(),
                SESSION_START_PY.into(),
            ),
            (
                ".agents/hooks/pre-web-check.py".into(),
                PRE_WEB_CHECK_PY.into(),
            ),
            (".agents/hooks/work-check.py".into(), WORK_CHECK_PY.into()),
            (
                ".agents/hooks/crosslink_config.py".into(),
                CROSSLINK_CONFIG_PY.into(),
            ),
            (".agents/hooks/heartbeat.py".into(), HEARTBEAT_PY.into()),
            (
                ".agents/mcp/safe-fetch-server.py".into(),
                SAFE_FETCH_SERVER_PY.into(),
            ),
            (
                ".agents/mcp/knowledge-server.py".into(),
                KNOWLEDGE_SERVER_PY.into(),
            ),
            (
                ".agents/mcp/agent-prompt-server.py".into(),
                AGENT_PROMPT_SERVER_PY.into(),
            ),
        ]);

        // Skill files (auto-discovered by build.rs)
        for (filename, content) in SKILL_FILES {
            files.push((format!(".agents/skills/{filename}"), content.to_string()));
        }

        // hooks.json template after prefix substitution
...
fn run_update(path: &Path, opts: &InitOpts<'_>) -> Result<()> {
    ...
    let template_files = managed_files(&prefix, opts.runtime);
...
pub fn run(path: &Path, opts: &InitOpts<'_>) -> Result<()> {
    ...
    let already_done = crosslink_exists
        && (!runtime.wants_claude() || claude_exists)
        && (!runtime.wants_antigravity() || agents_exists);

    if already_done && !force && !reconfigure {
    ...
    if opts.dry_run {
        ...
        let files = managed_files(&prefix, runtime);
    ...
    if runtime.wants_claude() && (!claude_exists || force) {
    ...
    // Create .agents directory and Antigravity CLI hooks (or update if force)
    if runtime.wants_antigravity() && (!agents_exists || force) {
        ui.step_start("Setting up Antigravity CLI hooks");

        let agy_hooks_dir = agents_dir.join("hooks");
        fs::create_dir_all(&agy_hooks_dir).context("Failed to create .agents/hooks directory")?;

        fs::write(agy_hooks_dir.join("prompt-guard.py"), PROMPT_GUARD_PY)
            .context("Failed to write .agents/hooks/prompt-guard.py")?;
        fs::write(agy_hooks_dir.join("post-edit-check.py"), POST_EDIT_CHECK_PY)
            .context("Failed to write .agents/hooks/post-edit-check.py")?;
        fs::write(agy_hooks_dir.join("session-start.py"), SESSION_START_PY)
            .context("Failed to write .agents/hooks/session-start.py")?;
        fs::write(agy_hooks_dir.join("pre-web-check.py"), PRE_WEB_CHECK_PY)
            .context("Failed to write .agents/hooks/pre-web-check.py")?;
        fs::write(agy_hooks_dir.join("work-check.py"), WORK_CHECK_PY)
            .context("Failed to write .agents/hooks/work-check.py")?;
        fs::write(
            agy_hooks_dir.join("crosslink_config.py"),
            CROSSLINK_CONFIG_PY,
        )
        .context("Failed to write .agents/hooks/crosslink_config.py")?;
        fs::write(agy_hooks_dir.join("heartbeat.py"), HEARTBEAT_PY)
            .context("Failed to write .agents/hooks/heartbeat.py")?;

        let agy_hooks_json_content = AGY_HOOKS_JSON.replace(PYTHON_PREFIX_PLACEHOLDER, &prefix);
        write_hooks_json_merged(&agents_dir.join("hooks.json"), &agy_hooks_json_content)
            .context("Failed to write .agents/hooks.json")?;

        let agy_mcp_dir = agents_dir.join("mcp");
        fs::create_dir_all(&agy_mcp_dir).context("Failed to create .agents/mcp directory")?;
        fs::write(
            agy_mcp_dir.join("safe-fetch-server.py"),
            SAFE_FETCH_SERVER_PY,
        )
        .context("Failed to write .agents/mcp/safe-fetch-server.py")?;
        fs::write(agy_mcp_dir.join("knowledge-server.py"), KNOWLEDGE_SERVER_PY)
            .context("Failed to write .agents/mcp/knowledge-server.py")?;
        fs::write(
            agy_mcp_dir.join("agent-prompt-server.py"),
            AGENT_PROMPT_SERVER_PY,
        )
        .context("Failed to write .agents/mcp/agent-prompt-server.py")?;

        let skills_dir = agents_dir.join("skills");
        fs::create_dir_all(&skills_dir).context("Failed to create .agents/skills directory")?;
        for (filename, content) in SKILL_FILES {
            fs::write(skills_dir.join(filename), content)
                .with_context(|| format!("Failed to write .agents/skills/{filename}"))?;
        }
    ...
    {
        let manifest_files = managed_files(&prefix, runtime);
...
mod tests {
    ...
    fn antigravity_opts(force: bool) -> InitOpts<'static> {
        InitOpts {
    ...
    fn both_opts(force: bool) -> InitOpts<'static> {
        InitOpts {
            runtime: AgentRuntime::Both,
            ..test_opts(force)
        }
    }
    ...
    fn test_init_antigravity_creates_agents_dir() {
        ...
        assert!(dir.path().join(".agents/mcp").exists());
        assert!(dir.path().join(".agents/skills").exists());
        assert!(dir.path().join(".agents/hooks/prompt-guard.py").exists());
    ...
    #[test]
    fn test_init_antigravity_does_not_create_claude_dir() {
        let dir = test_dir();
        run(dir.path(), &antigravity_opts(false)).unwrap();
    ...
    fn test_init_both_creates_both_dirs() {
        let dir = test_dir();
        run(dir.path(), &both_opts(false)).unwrap();
    ...
    fn test_init_both_no_regression_on_claude() {
        ...
        // All Claude Code files must still be present
        assert!(dir.path().join(".claude/settings.json").exists());
        assert!(dir.path().join(".claude/hooks/prompt-guard.py").exists());
        assert!(dir.path().join(".claude/mcp/safe-fetch-server.py").exists());
        assert!(dir.path().join(".mcp.json").exists());
    ...
    fn test_agents_md_created_when_missing() {
        ...
        assert!(content.contains("<!-- crosslink-managed-start -->"));
        assert!(content.contains("<!-- crosslink-managed-end -->"));
        assert!(content.contains("crosslink session start"));
    }
    ...
    fn test_agents_md_markers_replaced_on_force() {
        ...
        run(dir.path(), &antigravity_opts(true)).unwrap();

        let content = fs::read_to_string(&agents_md).unwrap();
        assert!(
            !content.contains("OLD CONTENT"),
            "Force should replace managed block"
        );
        assert!(content.contains("crosslink session start"));
    }
    ...
    #[test]
    fn test_hooks_json_valid_json() {
        let dir = test_dir();
        run(dir.path(), &antigravity_opts(false)).unwrap();

        let content = fs::read_to_string(dir.path().join(".agents/hooks.json")).unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&content).unwrap();
        assert!(parsed["crosslink"]["enabled"].as_bool() == Some(true));
        assert!(parsed["crosslink"]["PreInvocation"].is_array());
    }

    #[test]
    fn test_mcp_config_json_valid_json() {
        let dir = test_dir();
        run(dir.path(), &antigravity_opts(false)).unwrap();
        ...
        assert!(servers.contains_key("crosslink-knowledge"));
        assert!(servers.contains_key("crosslink-agent-prompt"));
    }

    #[test]
    fn test_hooks_json_preserves_user_hooksets() {
        let dir = test_dir();
        run(dir.path(), &antigravity_opts(false)).unwrap();

        // Add a user hookset to .agents/hooks.json
        let hooks_path = dir.path().join(".agents/hooks.json");
        let mut content: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&hooks_path).unwrap()).unwrap();
        content["my-custom-hooks"] = serde_json::json!({ "enabled": true });
        ...
        let result: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&hooks_path).unwrap()).unwrap();
        assert!(
            result.get("my-custom-hooks").is_some(),
            "User hookset should be preserved"
        );
        assert!(
            result["crosslink"]["enabled"].as_bool() == Some(true),
            "Crosslink hookset should be present"
        );
    }

    #[test]
    fn test_agent_runtime_from_str() {
        assert_eq!(
            "claude".parse::<AgentRuntime>().unwrap(),
            AgentRuntime::Claude
        );
        assert_eq!(
            "antigravity".parse::<AgentRuntime>().unwrap(),
            AgentRuntime::Antigravity
        );
        assert_eq!(
            "agy".parse::<AgentRuntime>().unwrap(),
            AgentRuntime::Antigravity
        );
        assert_eq!("both".parse::<AgentRuntime>().unwrap(), AgentRuntime::Both);
        assert!("unknown".parse::<AgentRuntime>().is_err());
    }

    #[test]
    fn test_antigravity_gitignore_includes_agents() {
        let dir = test_dir();
        run(dir.path(), &antigravity_opts(false)).unwrap();

        let content = fs::read_to_string(dir.path().join(".gitignore")).unwrap();
        assert!(content.contains(".agents/hooks/"));
        assert!(content.contains(".agents/skills/"));
        assert!(content.contains(".agents/mcp/"));
    }

    #[test]
    fn test_antigravity_deploys_skill_files() {
        let dir = test_dir();
        run(dir.path(), &antigravity_opts(false)).unwrap();

        let skills_dir = dir.path().join(".agents/skills");
        assert!(skills_dir.join("maintain.md").exists());
        assert!(skills_dir.join("design.md").exists());

        let maintain = fs::read_to_string(skills_dir.join("maintain.md")).unwrap();
