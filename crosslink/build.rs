...
fn main() {
    ...
    // Auto-discover and track all skill files in resources/antigravity/skills/
    println!("cargo:rerun-if-changed=resources/antigravity/skills/");
    let skills_dir = Path::new("resources/antigravity/skills");
...
fn generate_skills_file(skills_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in entries {
        let filename = entry.file_name().to_string_lossy().to_string();
        let rel_path = format!("resources/antigravity/skills/{filename}");
        println!("cargo:rerun-if-changed={rel_path}");
        ...
        skill_entries.push((filename, const_name));
    }
    ...
    writeln!(gen_file, "// Generated from resources/antigravity/skills/")?;
    writeln!(gen_file)?;

    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")?;
    let abs_skills_dir = Path::new(&manifest_dir).join("resources/antigravity/skills");

    for (filename, const_name) in &skill_entries {
        let abs_path = abs_skills_dir.join(filename);
        let abs_path_str = abs_path.to_string_lossy().replace('\\', "/");
        writeln!(
            gen_file,
            "pub(crate) const {const_name}: &str = include_str!(\"{abs_path_str}\");"
